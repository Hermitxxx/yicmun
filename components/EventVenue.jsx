        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {venues.map((venue, idx) => (
            <GlassCard
              key={venue.name}
              delay={idx * 0.15}
              className="group flex flex-col justify-between items-center text-center p-8 sm:p-12 relative overflow-hidden min-h-75 border border-[#00FF87]/20 hover:border-[#00FF87]/50"
            >
              ...
              <div className="w-full flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-arctic-blue border border-[#00D4FF]/30 text-xs font-mono text-[#00D4FF] tracking-wider uppercase">
                  <span>{venue.dates}</span>
                </div>
                <div className="w-8 h-8 rounded-full bg-arctic-blue border border-[#00FF87]/30 flex items-center justify-center text-[#00FF87] group-hover:scale-110 transition-transform">
              ...
              <div className="my-auto space-y-3 relative z-10">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-glacier-pink/10 border border-glacier-pink/30 text-[11px] font-mono text-glacier-pink tracking-widest uppercase">
                  <EyeOff className="w-3.5 h-3.5" />
                  <span>{venue.code || "LOCATION // CLASSIFIED"}</span>
                </div>

                <h3 className="font-bold text-2xl sm:text-3xl text-snow tracking-tight group-hover:text-[#00FF87] transition-colors duration-300">
                  <span className="inline-block relative">
                    <span className="opacity-90">{venue.name}</span>
                    <motion.span
                      animate={{ opacity: [0.3, 0.9, 0.3] }}
                      transition={{ duration: 2, repeat: Infinity }}
                      className="absolute -bottom-1 left-0 right-0 h-0.5 bg-linear-to-r from-[#00FF87] via-[#00D4FF] to-[#FF33B5]"
                    />
                  </span>
                </h3>
              </div>
              ...
              {/* Description */}
              <p className="mt-4 text-sm text-frost-white/75 leading-relaxed max-w-xs group-hover:text-snow transition-colors relative z-10">
                {venue.description}
              </p>

              {/* Bottom Secret Status Line */}
              <div className="w-full pt-4 mt-4 border-t border-frost-white/10 flex items-center justify-between text-xs font-mono text-[#00FF87]/80">
